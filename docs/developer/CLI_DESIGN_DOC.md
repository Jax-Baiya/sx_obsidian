...
**Problem:** Every single item has a decorative emoji. They add visual noise without aiding scannability. The eye has to parse both the emoji and the text, making the menu _harder_ to read quickly.
...
| **Navigation**          | Loop-based menu                                   | Stack-based router with breadcrumbs            | sx_obsidian is _more_ advanced      |
| **Session state**       | Basic (path history only)                         | `UIState` with session memory, wizard state    | sx_obsidian is _more_ advanced      |
...
**Key Takeaway:** sx_obsidian's TUI architecture is actually _more sophisticated_ than DocuMorph's (decorator registry, stack navigation, session state, component library). The improvements needed are mostly **surface-level UX polish** — not architectural changes.
